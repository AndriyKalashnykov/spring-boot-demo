...
public class HotelServiceHealth implements HealthIndicator {
  ...
  // application-specific health check according to
  // https://docs.spring.io/spring-boot/docs/current-SNAPSHOT/reference/htmlsingle/#production-ready-health
  @Override
  public Health health() {
    ...
    return Health.up()
        .withDetail(
            "details",
            "{ 'internals' : 'all good', 'profile' : '" + this.configuration.getName() + "' }")
        .status(Status.UP)
        .build();
  }
}
