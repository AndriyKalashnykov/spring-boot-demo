...
curl -X GET --silent 'http://localhost:8080/example/v1/hotels?page=0&size=10' --header 'Accept: application/json' --stderr - 2>&1 | jq .
curl -X GET --silent 'http://localhost:8080/example/v1/hotels?page=0&size=10' --header 'Accept: application/xml' --stderr - 2>&1 | xmllint --format -
